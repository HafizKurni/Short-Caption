...
from moviepy.video.fx import Resize  # <-- v2 effects (resize animation)


# -----------------------------
# Config
# -----------------------------
# Put your font files here (commit them to your repo)
DEFAULT_FONT_PATH = "fonts/Inter-SemiBold.ttf"

# If DEFAULT_FONT_PATH doesn't exist, we try these fallbacks
...
def translate_chunks_keep_timing(chunks: List[Dict], target_lang: str) -> List[Dict]:
    """
    Translation breaks word alignment. We keep chunk timing and approximate per-word timing
    by distributing time equally across translated words.
    """
...
# -----------------------------
# Karaoke layout + render (NO background box)
# -----------------------------
def compute_word_positions_centered(
    ...
    """
    For a SINGLE LINE caption: compute each word x so full line is centered.
    Adds 'x' and 'w' (pixel width).
    """
...
def render_full_line_rgba(
    ...
    extra_pad_y: int = 12,   # <-- increase if you still see clipping
) -> Tuple[np.ndarray, int, int]:
    """
    Render entire line into full-width RGBA strip with generous vertical padding.
    Returns: (rgba_strip, strip_h, y_text_offset)
    - y_text_offset is the Y inside strip where we start drawing text.
    """
    ascent, descent = font.getmetrics()
    line_h = ascent + descent

    sw = int(stroke_width)
    pad_y = sw + int(extra_pad_y)  # generous
    ...
    y_text = pad_y  # top padding area

    for w in words_with_pos:
        draw_text_with_stroke(
            draw, (int(w["x"]), int(y_text)), w["word"],
            font, fill, stroke_fill, sw
        )
...
def render_word_rgba(
    ...
    """
    Render single word to RGBA with padding (for stroke).
    Returns (rgba, total_w, total_h, inner_pad)
    """
    tmp = Image.new("RGBA", (10, 10), (0, 0, 0, 0))
    d = ImageDraw.Draw(tmp)
...
with colL:
    ...
    enable_translate = st.checkbox("Translate captions (timing becomes approximate)", value=False)
    ...
    pop_peak = st.slider("Pop peak scale", 1.0, 1.6, 1.22, 0.02)
    ...
    show_guides = st.checkbox("Show guide lines (preview)", value=True)
...
# -----------------------------
# Session cache (avoid re-transcribe on every slider change)
# -----------------------------
...
if video_file is not None:
    try:
        ...
        if frame is not None:
            ...
            # caption strip computed for current chunk (so we know strip_h for clamping)
            ...
            if chunk:
                ...
                # compute y target and clamp using strip_h to prevent cropping
                if y_mode == "Percent":
                ...
                if curw:
                    # find matching word layout by start time
                    match = None
                    for wp in words_pos:
                    ...
                    if match:
                        ...
                        # align word so its glyph start matches match["x"]
                        wx = int(match["x"]) - pad
                        wy = y + (y_text - pad)  # align inside strip (same baseline zone)
...
if render_btn:
    ...
    with st.status("Rendering…", expanded=True) as status:
        ...
        with VideoFileClip(video_path) as base:
            ...
            # We will clamp Y per chunk using strip_h (prevents clipping)
            for c in chunks:
                start = float(c["start"])
                end = float(c["end"])
                dur = max(0.10, end - start)
                ...
                # compute desired y and clamp using strip_h (CRUCIAL FIX)
                if y_mode == "Percent":
                    y = int(H * (y_percent / 100.0))
                else:
                    y = int(y_pos_px if y_pos_px is not None else int(H * 0.82))
                ...
                for wp in words_pos:
                    ...
                    # align highlight inside strip at same baseline zone
                    wx = int(wp["x"]) - pad
                    wy = y + (y_text - pad)
